...
class SyncPostgreSQL:
    def build_table(**args):
        ...
        if tablename == "segment":
            ...
            feature_columns = extract.feature_columns
        ...
        create_table = sync.get_create_table(interfaces, feature_columns)
        ...
        if tablename not in ["vertice", "segment", "time"]:
            ...
            if not (set(interfaces).issubset(set(table.columns)) and set(feature_columns).issubset(set(table.columns))):
        ...
        if tablename == "vertice":
            ...
            for c in list(set(dataframe.columns) - set(feature_columns)):
